...
def fetch_coinmetrics():
    """MVRV via CoinGecko market cap + blockchain.info realized cap proxy"""
    result = {}
    # Realized price proxy via blockchain.info (market price / MVRV ratio historically ~1.5-2.5)
    # Use CoinGecko market cap and estimate realized cap from supply * realized price
    # Best free source: glassnode public or blockchain.info stats
    d = get("https://blockchain.info/stats?format=json", timeout=8)
    cg = fetch_coingecko()
    if d and cg.get("price_usd"):
        # Blockchain.info gives market_price_usd and total supply info
        price = cg.get("price_usd")
        # Realized cap estimate: use 200d MA as realized price proxy * supply
        supply = cg.get("circulating_supply") or 19700000
        # From Kraken OHLC we compute MA200 as realized price proxy
        kr_meta, ohlc = fetch_kraken_ohlc()
        tech = compute_technicals(ohlc)
        ma200 = tech.get("ma_200d")
        if ma200 and price and supply:
            realized_cap = ma200 * supply
            mvrv = round(price / ma200, 4)
            nupl = round(1 - 1/mvrv, 4) if mvrv > 0 else None
            result.update({
                "mvrv": mvrv,
                "realized_cap": safe(realized_cap, 0),
                "nupl": nupl,
...
def fetch_bgeometrics():
    ...
    result = {}
    # Try bitcoin-data.com MCP API (v2)
    for field, endpoint in [
        ("asopr",     "/v1/asopr"),
        ("sopr_block","/v1/sopr"),
        ("nrpl_usd",  "/v1/nrpl-usd"),
        ("nrpl_btc",  "/v1/nrpl-btc"),
    ]:
        d = get(f"https://bitcoin-data.com{endpoint}", timeout=8)
        if d and isinstance(d, list) and d:
            last = d[-1]
            val = last.get("v") or last.get("value") or last.get("asopr") or last.get("sopr") or last.get("nrplUsd") or last.get("nrplBtc")
            if val is not None:
                result[field] = safe(val, 4)
        elif d and isinstance(d, dict):
            val = d.get("v") or d.get("value") or list(d.values())[0] if d else None
            if val is not None:
                result[field] = safe(val, 4)
    if result:
